...
## 项目分工
本次提交的代码主要面向「008 分支」的 [RAG功能] 开发，参与人员及核心职责如下：
...
| （协作人1，可选） | 1. 代码评审：聚焦 RAG 推理链路（rag_demo.py）与容错校验层（checker.py）的逻辑完整性；<br>2. Bug 修复：处理前端图谱交互异常（GraphView.tsx）与向量库数据一致性问题；<br>3. 辅助测试：编写单元测试脚本验证核心接口（/ask、/graph/overview）                                                                 | `tests/` 目录：<br>  - rag_test.py（RAG 问答链路单元测试）<br>  - graph_test.py（知识图谱数据接口测试）<br><br>`services/agent/` 目录：<br>  - rag_demo.py（接口调试与异常修复） |
