# [Minimum Cost to Make Two Binary Strings Equal][title]
...
You are allowed to apply the following operations any number of times (in any
order) to the strings `s` and `t`:
...
Flipping all the bits of `s` makes the strings equal, and the total cost is `3
* flipCost = 3 * 2 = 6`.
...
[title]: https://leetcode.com/problems/minimum-cost-to-make-two-binary-strings-equal
