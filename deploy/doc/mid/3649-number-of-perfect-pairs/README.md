...
are satisfied:
  ...
  * `i < j`
  * Let `a = nums[i]`, `b = nums[j]`. Then:     * `min(|a - b|, |a + b|) <= min(|a|, |b|)`    * `max(|a - b|, |a + b|) >= max(|a|, |b|)`
...
[title]: https://leetcode.com/problems/number-of-perfect-pairs
