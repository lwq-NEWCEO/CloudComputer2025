# [Total Score of Dungeon Runs][title]
...
remaining health points are **at least** `requirement[i]`, you earn **1
point** for that room.

Let `score(j)` be the number of **points** you get if you start with `hp`
health points and enter the rooms `j`, `j + 1`, ..., `n` in this order.

Return the integer `score(1) + score(2) + ... + score(n)`, the sum of scores
over all starting rooms.
...
As an example, `score(1) = 2` because you get 2 points if you start from room
1.
...
[title]: https://leetcode.com/problems/total-score-of-dungeon-runs
