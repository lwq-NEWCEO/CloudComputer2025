# [Find the Minimum Amount of Time to Brew Potions][title]
...
In a laboratory, `n` wizards must brew `m` potions _in order_. Each potion has
...
`jth` potion is `timeij = skill[i] * mana[j]`.
...
the timing must be _synchronized_ so that each wizard begins working on a
...
time `t = 52`, consider the case where the wizards started preparing the 1st
potion at time `t = 50`. At time `t = 58`, wizard 2 is done with the 1st
potion, but wizard 3 will still be working on the 0th potion till time `t =
60`.
