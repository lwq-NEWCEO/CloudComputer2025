# [Maximum Weighted K-Edge Path][title]
...
where `edges[i] = [ui, vi, wi]` indicates a directed edge from node `ui` to
`vi` with weight `wi`.
...
![](https://assets.leetcode.com/uploads/2025/04/09/screenshot-2025-04-10-at-061406.png)
  ...
  * There are two paths with `k = 1` edge:     * `0 -> 1` with weight `2 < t`.    * `0 -> 2` with weight `3 = t`, which is not strictly less than `t`.
...
![](https://assets.leetcode.com/uploads/2025/04/09/screenshot-2025-04-10-at-061442.png)
  ...
  * There are two paths with k = 1 edge:     * `0 -> 1` with weight `6 = t`, which is not strictly less than `t`.    * `1 -> 2` with weight `8 > t`, which is not strictly less than `t`.
