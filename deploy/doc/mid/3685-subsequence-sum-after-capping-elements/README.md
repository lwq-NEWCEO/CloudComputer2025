# [Subsequence Sum After Capping Elements][title]
...
Return a **0-indexed** boolean array `answer` of size `n`, where `answer[i]`
is `true` if it is possible when using `x = i + 1`, and `false` otherwise.
...
[title]: https://leetcode.com/problems/subsequence-sum-after-capping-elements
