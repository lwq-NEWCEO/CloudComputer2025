# [Minimum Time for K Connected Components][title]
...
= [ui, vi, timei]` indicates an undirected edge between nodes `ui` and `vi`
that can be removed at `timei`.

You are also given an integer `k`.

Initially, the graph may be connected or disconnected. Your task is to find
the **minimum** time `t` such that after removing all edges with `time <= t`,
...
[title]: https://leetcode.com/problems/minimum-time-for-k-connected-components
