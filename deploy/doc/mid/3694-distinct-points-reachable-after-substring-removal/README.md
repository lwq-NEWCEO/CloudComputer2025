# [Distinct Points Reachable After Substring Removal][title]
...
You **must** choose and remove **exactly one** contiguous substring of length
`k` from `s`. Then, start from coordinate `(0, 0)` and perform the remaining
moves in order.
...
`(-1, 1)` respectively. There are two distinct points `(-1, 1)` and `(-2, 0)`
so the answer is 2.
...
[title]: https://leetcode.com/problems/distinct-points-reachable-after-substring-removal
