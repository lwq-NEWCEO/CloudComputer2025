# [Minimum Time to Reach Destination in Directed Graph][title]
...
= [ui, vi, starti, endi]` indicates an edge from node `ui` to `vi` that can
**only** be used at any integer time `t` such that `starti <= t <= endi`.
