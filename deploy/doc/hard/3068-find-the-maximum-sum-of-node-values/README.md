# [Find the Maximum Sum of Node Values][title]
...
zero**) on the tree:
  ...
  * Choose any edge `[u, v]` connecting the nodes `u` and `v`, and update their values as follows:     * `nums[u] = nums[u] XOR k`    * `nums[v] = nums[v] XOR k`
