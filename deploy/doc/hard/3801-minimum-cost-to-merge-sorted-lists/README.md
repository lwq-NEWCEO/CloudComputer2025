# [Minimum Cost to Merge Sorted Lists][title]
...
new merged **sorted list** in **any** position. Repeat merges until only
**one** list remains.

Return an integer denoting the **minimum total cost** required to merge all
lists into one single sorted list.

The **median** of an array is the middle element after sorting it in non-
decreasing order. If the array has an even number of elements, the median is
the left middle element.
...
So `lists` becomes `[[1, 2, 3, 4, 5, 6, 7, 8]]`, and total cost is `6 + 12 =
18`.
...
[title]: https://leetcode.com/problems/minimum-cost-to-merge-sorted-lists
