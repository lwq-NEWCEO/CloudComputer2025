# [Minimum Time to Transport All Individuals][title]

## Description

You are given `n` individuals at a base camp who need to cross a river to
reach a destination using a single boat. The boat can carry at most `k` people
at a time. The trip is affected by environmental conditions that vary
...
Return the **minimum** total time required to transport all individuals. If it
is not possible to transport all individuals to the destination, return `-1`.
...
**Tags:** Array, Dynamic Programming, Bit Manipulation, Graph Theory, Heap (Priority Queue), Shortest Path, Bitmask DP
