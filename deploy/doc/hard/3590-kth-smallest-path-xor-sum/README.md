# [Kth Smallest Path XOR Sum][title]
...
each query, find the `kjth` **smallest distinct** path XOR sum among all nodes
in the **subtree** rooted at `uj`. If there are fewer than `kj` **distinct**
