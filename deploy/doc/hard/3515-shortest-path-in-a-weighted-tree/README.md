# [Shortest Path in a Weighted Tree][title]

## Description

You are given an integer `n` and an undirected, weighted tree rooted at node 1
with `n` nodes numbered from 1 to `n`. This is represented by a 2D array
...
distance from node 1 to `x` for the `ith` query of `[2, x]`.
...
**Constraints:**
  ...
  * `queries[i].length == 2` or `4`    * `queries[i] == [1, u, v, w']` or,    * `queries[i] == [2, x]`    * `1 <= u, v, x <= n`    * `(u, v)` is always an edge from `edges`.    * `1 <= w' <= 104`
