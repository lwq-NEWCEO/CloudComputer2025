# [Minimum Deletions to Make Alternating Substring][title]
...
A **substring** is **alternating** if no two **adjacent** characters are
**equal**. A substring of length 1 is always alternating.
...
**Constraints:**
  ...
  * `queries[i].length == 2` or `3`    * `queries[i] == [1, j]` or,    * `queries[i] == [2, l, r]`    * `0 <= j <= n - 1`    * `0 <= l <= r <= n - 1`
...
[title]: https://leetcode.com/problems/minimum-deletions-to-make-alternating-substring
