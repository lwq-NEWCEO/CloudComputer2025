# [Minimum Cost to Reach Every Position][title]
...
a specific amount to **swap** places. The cost to swap with person `i` is
given by `cost[i]`.
...
total cost to reach each position `i` in the line.
