...
Start iterating from the beginning of the array `nums`.
  ...
  * If `-1` is encountered, let `k` be the number of **consecutive** `-1`s seen so far (including the current `-1`),     * If `k` is less than or equal to the length of `seen`, append the `k`-th element of `seen` to `ans`.    * If `k` is strictly greater than the length of `seen`, append `-1` to `ans`.
...
[title]: https://leetcode.com/problems/last-visited-integers
