    ...
    def run(self):
        ...
        while not IS_SUCCESS:
            try:
                ...
                # 增加判空保护
                if 'data' not in content or 'question' not in content['data']:
                    print(f"Error fetching {self.title_slug}: {content}")
                    break
                ...
                if content['data']['question']['content'] != None:
                    ...
                    threadLock.acquire()
                    cursor.execute(
                        'INSERT INTO question (id, frontend_id, title, slug, difficulty, content, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
                        question_detail)
                    for tag in tags:
            ...
            # 若出现连接超时或连接错误则继续获取
                ...
                print(f"Unexpected error for {self.title_slug}: {e}")
                IS_SUCCESS = True  # 跳过错误
        cursor.close()
        conn.close()
...
class LeetcodeCrawler():
    ...
    def get_csrftoken(self):
        ...
        try:
            ...
            print("Failed to get csrf token, continuing without it (might affect login).")
    ...
    # 【修改点 1】: 根据难度生成子目录
    def get_diff_folder(self, difficulty):
        if difficulty == "Easy":
            return "easy"
    ...
    def get_ac_question_submission(self, filters):
        ...
        # ... (此处省略大部分未修改的爬取代码，保持原样即可) ...
        # 注意：为了节省篇幅，这里假设你保留了原文件里的 get_ac_question_submission 逻辑
        # 如果需要完整代码，请使用上面的 get_ac_question_submission
        # (实际上这里逻辑没变，只是为了生成时调用路径)

        # 补全 get_ac_question_submission 的原始逻辑，防止代码不完整
        slug_list = []
        for row in results:
            question_detail = {
                'id': row[0],
            ...
            while not IS_SUCCESS:
                try:
                    ...
                    if 'data' in content and 'submissionList' in content['data']:
                                ...
                                if cursor.fetchone()[0] == 0:
                                    ...
                                    while not IS_GET_SUBMISSION_SUCCESS:
                                        ...
                                        if not code:
                                            ...
                                            # continue # 这里continue可能会死循环，建议break或者忽略
                                            IS_GET_SUBMISSION_SUCCESS = True  # 强制跳过
                                        else:
                                            IS_GET_SUBMISSION_SUCCESS = True

                                    if code:
                                        submission_detail = (submission['id'], slug, submission['timestamp'],
                                                             submission['lang'], submission['runtime'], code)
                                        cursor.execute(
    ...
    def generate_question_markdown(self, question, path, has_get_code):
        # 【修改点 3】：构建分类路径 docs/easy/xxx
        diff_folder = self.get_diff_folder(question['difficulty'])
        base_path = os.path.join(path, diff_folder)

        # 确保 easy/mid/hard 文件夹存在
        if not os.path.exists(base_path):
            os.makedirs(base_path)

        # 题目文件夹
        text_path = os.path.join(base_path, "{:0>3d}-{}".format(question['frontedId'], question['slug']))
    ...
    def generate_questions_submission(self, path, filters):
        ...
        sql = """
              SELECT l.question_slug, l.code, l.language, q.frontend_id, max(l.timestamp) \
              FROM last_ac_submission_record as l, \
                   question as q
              WHERE l.question_slug == q.slug and q.status = 'ac' \
              GROUP BY l.question_slug \
              """
        cursor = self.conn.cursor()
        cursor.execute(sql)
        ...
        for submission in cursor:
            ...
            if not self.filter_question(question_detail, filters):
                continue
            # 【修改点 4】：传入难度信息
            self.generate_question_submission(path, submission, question_detail['difficulty'])

        cursor.close()
        filter_cursor.close()

    def generate_question_submission(self, path, submission, difficulty):
        # 【修改点 5】：代码文件也按难度存放
        diff_folder = self.get_diff_folder(difficulty)
        base_path = os.path.join(path, diff_folder)

        if not os.path.exists(base_path):
            os.makedirs(base_path)

        text_path = os.path.join(base_path, "{:0>3d}-{}".format(submission[3], submission[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--tags', nargs='+', help="Specify the tag")
    parser.add_argument('-v', '--verbose', action="store_true", default=False, help="Verbose output")
    parser.add_argument('-s', '--status', nargs='+', choices=['ac', 'notac', 'none'], help="Specify the probelms statu")
    parser.add_argument('-c', '--code', nargs='+', help="Code solution output path.")
    ...
    if argsDict.get('code') or argsDict.get('status'):
        if not (argsDict.get('username') and argsDict.get('password')):
            print("ERROR: username and password required for status/code!")
            sys.exit()
        else:
